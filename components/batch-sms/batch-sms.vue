	...
	<view>
		<uni-popup ref="smsPopup" type="center" @change="popupChange" :is-mask-click="false">
			<view class="sms-manager">
				<view class="sms-manager--header mb">消息提醒</view>
...
<!-- 					<uni-forms-item v-if="toType === 'user' && !isSelectedReceiver" label="目标对象" name="smsPreset" :rules="[{ required: true, errorMessage: '请选择目标对象' }]" required>
					...
					<uni-forms-item label="目标对象" v-if="toType === 'user' && isSelectedReceiver">
						<view>当前已选择{{ receiver.length }}人</view>
					</uni-forms-item>
                    <uni-forms-item label="目标对象" v-else>
                        <view>请选择需要提醒的用户</view>
                   </uni-forms-item>
<!-- 					<uni-forms-item label="目标对象" v-else-if="toType === 'userTags' ">
					...
					<uni-forms-item label="任务名称" name="name" required
						...
						<uni-easyinput v-model="smsDataModel.name" placeholder="请输入消息内容"/>
					</uni-forms-item>
	...
	methods: {
		...
		async sendSms(isPreview = false) {
            ...
            for(let i in receiver){
                let res = await db.collection('uni-id-users').where(`_id == "${receiver[i]}"`).field("nickname").get()
                let name=res.result.data[0].nickname
                // const id_token = uni.getStorageSync('uni_id_token');
                // console.log(id_token)
                // db.collection('uni-id-users')
                // 	.where(`_id == "${receiver[i]}"`)
                //     .field("nickname")
                // 	.get()
                // 	.then((res)=>{
